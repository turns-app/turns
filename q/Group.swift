...
class Group {
    ...
    init( name:String, callback:(group: NSDictionary) -> Void){
        let request = NSMutableURLRequest(URL: NSURL(string: "https://turns.website/groups.json?authentication_token=\(currentUser!.authenticationToken!)")!)
    ...
    class func all(goodcallback:(response:NSArray) -> Void, error:(error:NSError) -> Void ) {
        ...
        var url = NSURL(string: "https://turns.website/groups.json?authentication_token=\(token)")
