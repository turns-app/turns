...
class Group {
    ...
    init( name:String, callback:(group: NSDictionary) -> Void){
        let request = NSMutableURLRequest(URL: NSURL(string: "\(Environment.getBaseURL())/groups.json?authentication_token=\(currentUser!.authenticationToken!)")!)
    ...
    class func all(goodcallback:(response:NSArray) -> Void, error:(error:NSError) -> Void ) {
        ...
        var url = NSURL(string: "\(Environment.getBaseURL())/groups.json?authentication_token=\(token)")
    ...
    class func destroy(id:Int?){    
        let request = NSMutableURLRequest(URL: NSURL(string: "\(Environment.getBaseURL())/groups/\(id!).json?authentication_token=\(currentUser!.authenticationToken!)")!)
